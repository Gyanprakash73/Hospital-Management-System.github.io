...
<html>
    ...
    <body>
        <table class="table table-bordered">
            ...
            <tr bgcolor=red>
                <td ><font size=4 color=white>Update patient</font></td>
            </tr>
            <?php
            $rno=trim($_POST["rno"]);
            ...
            $sex=trim($_POST["sex"]);
            $error=0; if ($name=="") { $error=1; echo "<tr><td><font color=red size=4>Name can't empty</font></td></tr>"; }
            if ($addr=="")
            {
                $error=1;
            ...
            if ($error==0) 
            {
            mysqli_query($conn,"update patient set pname='".$name."',psex='".$sex."',paddr='".$addr."'where pno='".$rno."'");
